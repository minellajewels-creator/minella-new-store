...
function SuccessContent() {
  ...
  useEffect(() => {
    // Guard: if someone lands here directly with no valid method or txnid,
    // redirect them home instead of showing a fake success page.
    const validMethods = ["online", "cod", "failed"];
    if (!method || !validMethods.includes(method)) {
      router.replace("/");
      return;
    }
    // Online success must have a txnid — COD uses name param instead
    if (isOnline && !txnid) {
      router.replace("/");
    }
  }, [method, txnid, isOnline, router]);

  // Don't render anything until we know the method is valid
  ...
  return (
    ...
    >
      ...
      <p style={{ fontSize: 13, color: "var(--muted)", marginBottom: 32 }}>
        ...
        <a href="https://wa.me/919080014835" style={{ color: "var(--gold)" }}>
          +91 90800 14835
        </a>
      </p>
      <div
